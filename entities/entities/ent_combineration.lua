...
ENT.RationTypes     = {
    ...
    ["rationparcel_loyalist"] = COMBINE_RATION_LOYALIST,
    ["rationparcel_elevated"] = COMBINE_RATION_ELEVATED,
    ["rationparcel_highpriority"] = COMBINE_RATION_HIGHPRIORITY,
    ["rationparcel_standard"] = COMBINE_RATION_STANDARD,
    ["rationparcel_civpro"] = COMBINE_RATION_CIVPRO
}
...
function ENT:GetContextOptions(ply, interact)
    ...
    table.insert(tab, {
        ...
        Name = "Collect Ration",
        Callback = function()
            local rationtime = (ply:LastRationTime() + 10800) - os.time() 
		...
		local ok, flipped, x, y = inventory:FindFit(2,2)

		if not ok then
			ply:SendChat("NOTICE", "You cannot collect your ration. Your inventory is currently full.")
                	self:EmitSound("buttons/button10.wav", 65)
				return false
		end	
