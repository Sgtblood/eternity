...
GM.WeaponSlotOverrides["eternity_admin_explosives"] = {3, 6}
...
-- GM:RegisterHUDSection("armorbar", function(colors, config, cache)
	-- local style = GAMEMODE:GetSetting("hud_armorbar")

	--if style == ARTYPE_BAR then
		--local percentage = math.Clamp(LocalPlayer():Armor() / LocalPlayer():GetMaxArmor(), 0, 1)

		--cache.Armor = math.Approach(cache.Armor, percentage, FrameTime())
		...
		--surface.SetDrawColor(colors.ItemAllow)
		--surface.DrawRect(config.Offset, y + margin, w * cache.Armor, h - margin)
--	end
--end)
