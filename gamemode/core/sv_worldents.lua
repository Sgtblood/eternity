...
function GM:LoadWorldEnts()
	...
	for _, v in ipairs(dbal.Query("connor4_Blob", "SELECT * FROM $worldents WHERE MapName = ?", game.GetMap())) do
...
function GM:SaveWorldEnt(ent)
	...
	if ent:IsReady() then
		-- Saved before
		dbal.Update("connor4_Blob", "$worldents", {
			MapPos = pos,
			CustomData = data
		...
		coroutine.WrapFunc(function()
			local id = dbal.Insert("connor4_Blob", "$worldents", {
				Class = ent:GetClass(),
				MapName = game.GetMap(),
				MapPos = pos,
				CustomData = data
...
function GM:DeleteWorldEnt(ent)
	...
	coroutine.WrapFunc(function()
		dbal.Query("connor4_Blob", "DELETE FROM $worldents WHERE ID = ?", ent:GetEntityID())
