...
function CLASS:CanAccept(item)
	...
	if self.StoreBlock ~= STOREBLOCK_NONE then
		if self.StoreBlock == item.StoreBlock then
			return false
		end

		if self.StoreBlock == STOREBLOCK_INVENTORY and table.Count(item.Inventories) > 0 then
			return false
		end
	end
...
function CLASS:CanInteract(ply)
	...
	elseif self.GridType == STORE_ITEM then
		...
		return item and item:CanInteract(ply) and item:CanInteractWithChild(ply)
