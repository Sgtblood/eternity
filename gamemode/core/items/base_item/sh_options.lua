function ITEM:CreateOptions(ply)
	...
	for _, v in pairs(self:GetOptions(ply)) do
		...
		popup:AddOption(option, function()
			if v.Client then
...
function ITEM:GetOptions(ply)
	...
	if table.Count(self.Inventories) > 0 and self:CanInteractWithChild(ply) then
		table.insert(tab, {
			Name = "Open Inventory",
			Client = function()
				self.InventoryPopup = GAMEMODE:OpenGUI("InventoryPopup", self:GetName(), self.Inventories)
			end
		})
	end
