...
function GM:HandleItemRemappings()
	...
	for k, v in pairs(config) do
		...
		dbal.Update("connor4_Blob", "$items", {Class = v}, "Class = ?", k)
	end
end
...
function GM:LoadInventoryItems(inventory)
	for _, v in ipairs(dbal.Query("connor4_Blob", "SELECT * FROM $items WHERE StoreType = ? AND StoreID = ? AND StoreName = ?", inventory.GridType, inventory.GridID, inventory.GridName)) do
		self:LoadItem(v, inventory)
	end
end

function GM:LoadWorldItems()
	for _, v in ipairs(dbal.Query("connor4_Blob", "SELECT * FROM $items WHERE StoreType = ? AND StoreName = ?", STORE_WORLD, game.GetMap())) do
		self:LoadItem(v)
	end
end
...
function GM:CreateItem(classname)
	...
	local id = dbal.Insert("connor4_Blob", "$items", {Class = classname}).LastInsert
	local data = dbal.Query("connor4_Blob", "SELECT * FROM $items WHERE id = ?", id)[1]
...
function GM:DeleteItem(id)
	...
	dbal.Query("connor4_Blob", "DELETE FROM $items WHERE id = ?", id)
