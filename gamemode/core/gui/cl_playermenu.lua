...
function PANEL:CreateInventory()
	...
	if table.Count(LocalPlayer():GetActiveSpecies().EquipmentSlots2) > 0 then
	...
	if self.Equipment2 then
		self.Armor = self.Equipment2:Add("eternity_label")
	else
		self.Armor = self.Equipment:Add("eternity_label")
	end

	self.Armor:DockMargin(2, 0, 0, 0)
	self.Armor:Dock(BOTTOM)
...
function PANEL:CreateSettings()
	...
	self.StopServerMusic = right:Add("eternity_button")
	self.StopServerMusic:SetSize(120, 22)
	self.StopServerMusic:SetText("Stop Serverside Sound")
	self.StopServerMusic:DockMargin(0, 5, 0, 0)
	self.StopServerMusic:Dock(TOP)

	self.StopServerMusic.DoClick = function(pnl)
		RunConsoleCommand("rp_stopmusic")
		RunConsoleCommand("rp_stopeffect")
	end
