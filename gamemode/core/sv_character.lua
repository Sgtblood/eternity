...
function meta:CreateNewCharacter(data, language)
	...
	local id = dbal.Insert("connor4_Blob", "$chars", data).LastInsert
...
function meta:LoadCharacter(id)
	...
	local data = dbal.Query("connor4_Blob", "SELECT * FROM $chars WHERE id = ?", id)[1]
...
function meta:DeleteCharacter(id)
	dbal.Update("connor4_Blob", "$chars", {Deleted = 1}, "id = ?", id)
	dbal.Query("connor4_Blob", "DELETE FROM $items WHERE StoreType = ? AND StoreID = ?", STORE_PLAYER, id)
