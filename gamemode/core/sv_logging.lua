function GM:WriteLog(identifier, data)
	...
	local time = os.time()
	local func = function()
		local id = dbal.Insert("connor4_Blob", "$logs", {
		...
		for _, v in pairs(data) do
			...
			if v._meta == META_CHAR and not charcache[v.CharID] then
				...
				dbal.Insert("connor4_Blob", "$logchars", {logid = id, charid = v.CharID})
			elseif v._meta == META_ITEM and not itemcache[v.ItemID] then
				itemcache[v.ItemID] = true

				dbal.Insert("connor4_Blob", "$logitems", {logid = id, itemid = v.ItemID})
			elseif v._meta == META_PLY and v.SteamID and not plycache[v.SteamID] then
				plycache[v.SteamID] = true

				dbal.Insert("connor4_Blob", "$logplayers", {logid = id, steamid = v.SteamID})
			end
		end
...
function GM:ReadLogs(filters)
	...
	local data = dbal.Query("connor4_Blob", query, unpack(args))
