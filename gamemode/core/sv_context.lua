...
netstream.Hook("CancelGesture", function(ply)
...
netstream.Hook("PlayGesture", function(ply, data)
	...
	local gesture = gestures.Options[data.Gesture]
	...
	if gesture and IsValid(ply) and ply:IsPlayer() then
		netstream.Send("PlayGesture", {
			HoldAnimation = gestures.HoldAnimation,
			Gesture = gesture,
			Player = ply
		})
	end
end, {
	Submenu = {Type = TYPE_NUMBER, Optional = true},
