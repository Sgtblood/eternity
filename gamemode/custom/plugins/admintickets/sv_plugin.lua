...
function GM:PlayerDisconnected(ply)
	if ply.ActiveTicket then hook.Run("PlayerTicketDestroyed", ply.ActiveTicket) end
end
...
net.Receive("CloseTicket", function(len, ply)
	...
	dbal.RawQuery("connor4_Blob", Format(INSERT_QUERY, ticket.initiator:SteamID(), ticket.assigned:SteamID()))
...
hook.Add("OnDBALConnected", "CreateTicketsTable", function(key)
	...
	dbal.RawQuery("connor4_Blob", TABLE_QUERY)
end)
...
console.AddCommand("rpa_viewtickets", function(ply)
	dbal.RawQuery("connor4_Blob", GET_TICKET_STATS, function(data)
