ITEM = class.Create("base_clothing")

ITEM.Name 				= "Infestation Control Team Leader Suit"
ITEM.Description 		= "A bright orange suit meant to protect you from your toxic friends."

ITEM.Model 				= Model("models/tnb/halflife2/citizens/items/item_hazmat.mdl")
ITEM.Skin 				= 1

ITEM.Width 				= 3
ITEM.Height 			= 3

ITEM.OutlineColor       = Color(134, 197, 218)

ITEM.ArmorOverride      = true
ITEM.ArmorLevel         = ARMOR_LIGHT

ITEM.EquipmentSlots 	= {EQUIPMENT_TORSO}

ITEM.ItemGroup 			= "Clothing"

ITEM.Inventories        = {
    ["Left Pocket"] = {1, 2},
    ["Right Pocket"] = {1, 2}
}

ITEM.Filtered           = true

if SERVER then
    function ITEM:SetupHands(ply, ent)
        ent:SetModel("models/weapons/c_arms_combine.mdl")
        ent:SetSkin(0)
        ent:SetBodyGroups("00000000")
    
        return true
    end

    function ITEM:GetModelData(ply)
        local gender = GAMEMODE:GetGenderString(ply:CharModel())

		return {
            torso = {
                Model = string.format("models/tnb/halflife2/%s_suit_hazmat_orange.mdl", gender)
            }
        }
	end

     function ITEM:PostModelData(ply, data)
        data.legs = nil
        data.hands = nil

        return data
    end

end

return ITEM
