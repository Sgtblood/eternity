...
netstream.Hook("UnitActivityFetchAllClient", function(data)
    if ACTIVE and IsValid(ACTIVE) and ispanel(ACTIVE) and ACTIVE:IsValid() then
        ...
        for k,v in pairs(data.Data) do
            ...
            elseif v.SpeciesState == "police_divisional" then
                name = string.format("%s.DvL.%s", squad, cid)
            elseif v.SpeciesState == "police_commissioner" then
                name = string.format("COM.%s", squad, cid)
