netstream.Hook("UnitActivityRemoveID", function(ply, data)
    ...
    local data = dbal.Query("connor4_Blob", "SELECT SpeciesState, id FROM $chars WHERE id = ?", char)[1]
    ...
    if data.SpeciesState == "police_divisional" and ply:SpeciesState() == "police_captain" then
        remove = true
    elseif data.SpeciesState == "police_squadleader" and ply:HasCombinePermission(COMBINE_ADMINS_PROMOTE_SQL) then
        remove = true
    ...
    if remove then
        dbal.Update("connor4_Blob", "$chars", {SpeciesState = "citizen"}, "id = ?", char)
...
netstream.Hook("UnitActivityFetchAll", function(ply)
    ...
    local data = dbal.Query("connor4_Blob", "SELECT LastFlagged, CitizenNumber, CombineRank, CombineSquad, SpeciesState, id FROM $chars WHERE SpeciesState LIKE 'police_%'")
    local valid = {}
    ...
    for k,v in pairs(data) do
        ...
        elseif (v.SpeciesState == "police_squadleader" or v.SpeciesState == "police_divisional") and (ply:HasCombinePermission(COMBINE_ADMINS_PROMOTE_SQL) or ply:SpeciesState() == "police_captain") then
            table.insert(valid, v)
...
netstream.Hook("UnitManagementDemoteClass", function(ply, data)
    ...
    if state.Flagged.CombineLevel and target.Flagged.CombineLevel and state.Flagged.CombineLevel >= target.Flagged.CombineLevel then
        if unit:SpeciesState() == "police_divisional" and ply:SpeciesState() == "police_captain" then
            unit:SetSpeciesState("police_squadleader")
        elseif unit:SpeciesState() == "police_squadleader" and ply:HasCombinePermission(COMBINE_ADMINS_PROMOTE_SQL) then
...
netstream.Hook("UnitManagementPromoteClass", function(ply, data)
    ...
    if state.Flagged.CombineLevel and target.Flagged.CombineLevel and state.Flagged.CombineLevel >= target.Flagged.CombineLevel then
        ...
        elseif unit:SpeciesState() == "police_unit" and ply:HasCombinePermission(COMBINE_ADMINS_PROMOTE_SQL) then
            unit:SetSpeciesState("police_divisional")
