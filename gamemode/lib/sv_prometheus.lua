hook.Add("OnDBALConnected", "prometheus.OnDBALConnected", function(key)
	if key ~= "prometheus" then return end
	timer.Create("CheckDonations", 300, 0, function() coroutine.WrapFunc(function() prometheus.Update() end) end)
end)

local function give(ply, price, actions)
	if not actions.custom_action then return end
...
local function take(ply, actions)
	if not actions.custom_action then return end
	if not actions.custom_action.code_after then return end
...
local function process(tab)
	...
	if table.Count(update) > 0 then dbal.Update("connor4_Blob", "actions", update, "id = ?", tab.id) end
end
...
function Update()
	...
	for k, v in ipairs(dbal.Query("connor4_Blob", query, id)) do
		process(v)
	end
end