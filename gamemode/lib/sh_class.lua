...
function Create(name)
	local base = nil
	if name then base = Get(name) end
	local class = {}
	local instance_meta = {
...
function GetNetworked(group, id)
	if not Networked[group] then return end
...
function RootClass:Create(...)
	local instance = setmetatable({
		Proxy = {}
	}, getmetatable(self).InstanceMeta)
...
function RootClass:IsTypeOf(target)
	...
	if not isstring(target) then target = target:GetClassName() end
	while class do
...
function RootClass:ParentCall(func, ...)
	...
	if base and base[func] then val = {base[func](self, unpack({...}))} end
	self.__Depth = self.__Depth - 1
	if self.__Depth == 0 then self.__Depth = nil end
	return unpack(val)
end
...
function CLASS:Destroy()
	...
	if SERVER then self:DeleteOnClients() end
end
...
if SERVER then
	function CLASS:OnCreated()
		if not Networked[self.NetworkGroup] then Networked[self.NetworkGroup] = {} end
	...
	function CLASS:SetNetworked(bool)
		...
		if self.__Networked == bool then return end
	...
	function CLASS:CreateOnClients(targets)
		...
		for _, k in pairs(table.GetKeys(self.Proxy)) do
			if self.NetworkBlacklist[k] then continue end
			keyvalues[k] = self.Proxy[k]
		end

		if GetConVar("debug_replicated"):GetBool() then log.Default(string.format("[Replicated] Creating on network: %s[%s][%s]", self:GetClassName(), self.NetworkGroup, self.NetworkID)) end
	...
	function CLASS:DeleteOnClients()
		if GetConVar("debug_replicated"):GetBool() then log.Default(string.format("[Replicated] Deleting from network: %s[%s][%s]", self:GetClassName(), self.NetworkGroup, self.NetworkID)) end
	...
	function CLASS:NewIndex(key, value)
		if self:GetNetworked() and not self.NetworkBlacklist[key] then
			if GetConVar("debug_replicated"):GetBool() then log.Default(string.format("[Replicated] Setting %s on %s[%s][%s]", key, self:GetClassName(), self.NetworkGroup, self.NetworkID)) end
	...
	netstream.Hook("RequestReplicated", function(ply)
		for _, group in pairs(Networked) do
			for _, v in pairs(group) do
				if not v:GetNetworked() then continue end
...
if CLIENT then
	hook.Add("InitPostEntity", "class.InitPostEntity", function() netstream.Send("RequestReplicated") end)
	netstream.Hook("CreateReplicated", function(data)
		...
		if not Networked[instance.NetworkGroup] then Networked[instance.NetworkGroup] = {} end
