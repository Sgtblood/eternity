hook.Add("OnDBALConnected", "playerreg.OnDBALConnected", function(key)
	...
	dbal.RegisterTable("connor4_Blob", "playerreg", "reg", {
	...
	dbal.RegisterTable("connor4_Blob", "playerreg_ips", "reg_ips", {
		SteamID = {Type = "VARCHAR(30)", PK = true},
		IP = {Type = "INT UNSIGNED", PK = true},
		Timestamp = {Type = "INT"}
	})

	dbal.RegisterTable("connor4_Blob", "playerreg_nicks", "reg_nicks", {
		SteamID = {Type = "VARCHAR(30)", PK = true},
		Nick = {Type = "VARCHAR(32)", PK = true},
		Timestamp = {Type = "INT"}
	})

	dbal.RegisterTable("connor4_Blob", "playerreg_gameids", "reg_gameids", {
		SteamID = {Type = "VARCHAR(30)", PK = true},
		GameID = {Type = "INT", PK = true},
		Timestamp = {Type = "INT"}
	})

	dbal.RegisterTable("connor4_Blob", "playerreg_lenders", "reg_lenders", {
		SteamID = {Type = "VARCHAR(30)", PK = true},
	...
	dbal.Query("connor4_Blob", "CREATE INDEX SteamID ON $reg_ips (SteamID)")
	dbal.Query("connor4_Blob", "CREATE INDEX SteamID ON $reg_nicks (SteamID)")
	dbal.Query("connor4_Blob", "CREATE INDEX SteamID ON $reg_gameids (SteamID)")
	dbal.Query("connor4_Blob", "CREATE INDEX SteamID ON $reg_lenders (SteamID)")
...
function Update(ply)
	...
	dbal.Insert("connor4_Blob", "$reg", {SteamID = steamid, LastNick = nick, LastIP = ip, FirstOn = timestamp, LastOn = timestamp}, true)

	dbal.Query("connor4_Blob", "INSERT INTO $reg_ips (SteamID, IP, Timestamp) VALUES (?, ?, ?) ON DUPLICATE KEY UPDATE Timestamp = ?", steamid, ip, timestamp, timestamp)
	dbal.Query("connor4_Blob", "INSERT INTO $reg_nicks (SteamID, Nick, Timestamp) VALUES (?, ?, ?) ON DUPLICATE KEY UPDATE Timestamp = ?", steamid, nick, timestamp, timestamp)

	dbal.Update("connor4_Blob", "$reg", {LastNick = nick, LastIP = ip, LastOn = timestamp}, "SteamID = ?", steamid)
	...
	if #key > 0 then
		...
		dbal.Query("connor4_Blob", "INSERT INTO $reg_lenders (SteamID, Lender, Timestamp) VALUES (?, ?, ?) ON DUPLICATE KEY UPDATE Timestamp = ?", ply:SteamID(), util.SteamIDFrom64(lender), timestamp, timestamp)
	end
end
...
function UpdateTimer(userid)
	...
	dbal.Query("connor4_Blob", "UPDATE $reg SET PlayTime = PlayTime + ? WHERE SteamID = ?", time, tab.SteamID, stub)
end

function Exists(steamid)
	return tobool(dbal.Query("connor4_Blob", "SELECT * FROM $reg WHERE SteamID = ?", steamid)[1])
end

function GetData(ply, steamid)
	local data = dbal.Query("connor4_Blob", "SELECT * FROM $reg WHERE SteamID = ?", steamid)[1]
	...
	for _, v in ipairs(dbal.Query("connor4_Blob", "SELECT IP, Timestamp FROM $reg_ips WHERE SteamID = ?", steamid)) do
	...
	for _, v in ipairs(dbal.Query("connor4_Blob", "SELECT Nick, Timestamp FROM $reg_nicks WHERE SteamID = ?", steamid)) do
	...
	for _, v in ipairs(dbal.Query("connor4_Blob", "SELECT RPName FROM $chars WHERE SteamID = ? AND Deleted = 0", steamid)) do
		send(ply, "    %s", v.RPName)
	end
	...
	for _, v in ipairs(dbal.Query("connor4_Blob", "SELECT GameID, Timestamp FROM $reg_gameids WHERE SteamID = ?", steamid)) do
		send(ply, "    %s - %s", v.GameID, timesince(v.Timestamp))
	end

	local lenders = dbal.Query("connor4_Blob", "SELECT Lender, Timestamp FROM $reg_lenders WHERE SteamID = ?", steamid)
...
function GetByIP(ply, steamid)
	...
	local data, matches = filter("IP", dbal.Query("connor4_Blob", query, steamid, steamid))
...
function GetByGameID(ply, steamid)
	...
	local data, matches = filter("GameID", dbal.Query("connor4_Blob", query, steamid, steamid))
...
netstream.Hook("GameID", function(ply, data)
	...
	dbal.Query("connor4_Blob", "INSERT INTO $reg_gameids (SteamID, GameID, Timestamp) VALUES (?, ?, ?) ON DUPLICATE KEY UPDATE Timestamp = ?", steamid, data.GameID, timestamp, timestamp)
end)
